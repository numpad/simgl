...
public:
	...
	void render() {
		this->currot += rotspeed;
		model = glm::translate(model, this->position);
		model = glm::scale(glm::mat4(1.0f), glm::vec3(2.0, 0.25f, 0.5f));
		model = glm::rotate(model, glm::radians(currot), glm::vec3(0.2f, 0.8f, -0.64f));
		s.use();
...
int main(int argc, char *argv[]) {
	...
	window.on_update([&](sgl::window &, nk_context *ctx) {
		...
		c.move(glm::vec3(0.025f * cos(ctime), 0.025f * sin(ctime), 0.0f));
		c2.move(glm::vec3(0.025f * sin(ctime), 0.025f * cos(ctime), 0.0f));
		ctime += 0.1f;
